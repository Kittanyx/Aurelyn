...
const { useEffect, useRef, useState, useLayoutEffect, useCallback, useMemo } = React;

// Magnetic button — pulls toward cursor on hover
function Magnetic({ children, strength = 22, className = "", style, onClick, type, as = "button", href, ...rest }){
  ...
  useEffect(()=>{
    ...
    function tick(){
      ...
      if(Math.abs(ix - tx) > 0.1 || Math.abs(iy - ty) > 0.1){ raf = requestAnimationFrame(tick); }
      else { raf = 0; el.style.transform = `translate3d(${tx}px, ${ty}px, 0)`; }
    }
  ...
  const Tag = as;
  return (
    <Tag ref={ref} className={"mag " + className} style={style} onClick={onClick} type={type} href={href} {...rest}>
...
function Reveal({ children, as: Tag = "div", className = "", stagger = false, ...rest }){
  const ref = useRef(null);
  useEffect(()=>{ if(ref.current && window.__reveal) window.__reveal(ref.current); }, []);
...
function SplitChars({ text, className = "", baseDelay = 0, step = 22 }){
  const chars = useMemo(()=> Array.from(text || ""), [text]);
  return (
    <span className={"split " + className} aria-label={text}>
      {chars.map((c, i) => (
        <span key={i} className="split-char" style={{ display:"inline-block", transitionDelay: (baseDelay + i*step) + "ms" }}>
          {c === " " ? "\u00A0" : c}
        </span>
      ))}
    </span>
...
Object.assign(window, { Magnetic, Reveal, SplitChars, useScrollY, useElementProgress });
