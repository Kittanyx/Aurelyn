...
function Hero({ t, lang }){
  ...
  return (
    <section className="hero" ref={heroRef} aria-label="hero" style={{ padding: 0 }}>
      <style>{`
        .hero{
        ...
        <div className="hero-mid">
          <div>
            <h1 className="hero-h1">
              ...
              <span className="row stroked">{t.hero.title2}</span>
              <span className="row">{t.hero.title3}</span>
            </h1>
            <p className="hero-sub">{t.hero.sub}</p>
            <div className="hero-cta-row">
              <Magnetic onClick={()=>document.getElementById('contact')?.scrollIntoView({behavior:'smooth', block:'start'})}>
                {t.hero.cta} <span className="arrow"></span>
              </Magnetic>
              <Magnetic className="dark" onClick={()=>document.getElementById('cases')?.scrollIntoView({behavior:'smooth', block:'start'})}>
                {t.hero.cta2}
              </Magnetic>
            </div>
            <div className="hero-tags">
              <span>{t.hero.label1}</span>
              <span>{t.hero.label2}</span>
              <span>{t.hero.label3}</span>
            </div>
          </div>
          ...
          <div className="hero-shape-wrap">
            ...
            <div className="shape-badge">
              <span className="red">●</span> BUILT FOR<br/>
              SPEED.<br/>
              ENGINEERED<br/>
              FOR HUMANS.
            </div>
...
function ServiceCube(){
  ...
  useEffectH(()=>{
    const cube = cubeRef.current; if(!cube) return;
    let rotY = 0, rotX = -16;
    let velY = 60; // deg/sec auto Y
    let velX = 40;  // deg/sec auto X (slight tilt drift)
    let dragging = false;
    let lastX = 0, lastY = 0;
    let dragVelY = velY, dragVelX = velX;
  ...
  const half = size/2;
  const faces = [
    { tf: `translateZ(${half}px)`, label:"WEB", num:"01", icon:<IconWeb/> },
    { tf: `rotateY(90deg) translateZ(${half}px)`, label:"SAAS", num:"02", icon:<IconSaaS/> },
    { tf: `rotateY(180deg) translateZ(${half}px)`, label:"BOTS", num:"03", icon:<IconBot/> },
    { tf: `rotateY(-90deg) translateZ(${half}px)`, label:"API", num:"04", icon:<IconAPI/> },
    { tf: `rotateX(90deg) translateZ(${half}px)`, label:"AURELYN", num:"●", icon:<IconMark/> },
    { tf: `rotateX(-90deg) translateZ(${half}px)`, label:"2026", num:"∞", icon:<IconInfinity/> },
  ];
  return (
    <div className="cube-stage" ref={stageRef} style={{ cursor:'grab', touchAction:'none' }}>
      <div className="cube" ref={cubeRef} style={{ animation:'none' }}>
        {faces.map((f, i) => (
          <div className="face" key={i} style={{ transform: f.tf }}>
            {f.icon}
            <span className="label">{f.label}</span>
            <span className="corner">{f.num}</span>
          </div>
        ))}
      </div>
    </div>
...
const IconWeb = () => (
  <svg className="icon" viewBox="0 0 64 64" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="square">
...
const IconSaaS = () => (
  <svg className="icon" viewBox="0 0 64 64" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="square">
    <rect x="6" y="14" width="22" height="14" rx="1"/>
    <rect x="32" y="14" width="26" height="14" rx="1"/>
    <rect x="6" y="32" width="26" height="18" rx="1"/>
    <rect x="36" y="32" width="22" height="18" rx="1"/>
    <line x1="11" y1="42" x2="11" y2="46"/>
    <line x1="16" y1="38" x2="16" y2="46"/>
...
const IconAPI = () => (
  <svg className="icon" viewBox="0 0 64 64" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="square">
    ...
    <line x1="20" y1="32" x2="44" y2="20"/>
    <line x1="20" y1="32" x2="44" y2="44"/>
    <polyline points="6 22 14 26 14 38 6 42"/>
    <polyline points="58 12 50 8 42 12"/>
  </svg>
);
const IconMark = () => (
  <svg className="icon" viewBox="0 0 64 64" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="square">
    <polygon points="32 8 56 50 8 50"/>
    <line x1="20" y1="50" x2="32" y2="28"/>
    <line x1="44" y1="50" x2="32" y2="28"/>
  </svg>
);
const IconInfinity = () => (
  <svg className="icon" viewBox="0 0 64 64" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
    <path d="M14 32 C14 22, 26 22, 32 32 C38 42, 50 42, 50 32 C50 22, 38 22, 32 32 C26 42, 14 42, 14 32 Z"/>
  </svg>
);
